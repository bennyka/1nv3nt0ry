...
//Rare exceptions to indicate a bug in the _inventory code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_inventory = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_inventory]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.inventory.module"
